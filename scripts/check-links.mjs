...
//   /favicon.svg -> dist/favicon.svg           (exact file for assets)
...
function classify(href) {
  ...
  // Absolute URLs: only liberatingscripture.org is internal; reduce to its pathname.
  if (/^[a-z][a-z0-9+.-]*:/i.test(raw)) {
    ...
    if (url.hostname !== "liberatingscripture.org") return null;
...
async function resolveTargetFile(pathname) {
  ...
  const candidates = [
    base, // exact file (assets: /favicon.svg, /_astro/x.css)
